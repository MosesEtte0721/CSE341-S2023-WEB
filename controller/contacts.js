...
const createContact = async (req, res) => {
  ...
  if (data.acknowledged) {
    ...
    res
      .status(500)
      .json(data.error || "An error occurred while creating the document");
  }
};
  ...
  try{
      ...
      if (connect.modifiedCount > 0) {
        ...
        res
          .status(500)
          .json({
            message: connect.error || "error occurred while updating the document",
          });
      }
  }
  catch(error) {
...
const deleteContact = async (req, res) => {
  ...
  if (connect.deletedCount > 0) {
    ...
    res
      .status(500)
      .send(connect.error || "error occurred while deleting the document.");
  }
};
